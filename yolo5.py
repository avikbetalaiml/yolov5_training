...
class yolov5_training:

    def __init__(self, project_id=None, epochs=None, frame_work=None, algo=None):
    ...
    def train(self, hyp, opt, device, tb_writer=None):
            logger.info(f'Hyperparameters {hyp}')
            log_dir = Path(tb_writer.log_dir) if tb_writer else Path(opt.logdir) / 'evolve'  # logging directory
            wdir = log_dir / 'weights'  # weights directory
            os.makedirs(wdir, exist_ok=True)
    ...
    def mlflow_init(self):
        ...
        insert_data_obj = yolo5_db()
        insert_data = insert_data_obj.train_comp(run_id = run_id, project_id = self.project_id, epochs = self.epochs, frame_work = self.frame_work, algo = self.algo)
        ...
        if not opt.evolve:
            ...
            self.train(hyp, opt, device, tb_writer)
